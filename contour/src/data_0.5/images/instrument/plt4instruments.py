...
def merge_images(image_files, prefixes, output_filename):
    ...
    for image, prefix in zip(image_files, prefixes):
        ...
        if prefix in ['center_', 'side_']:  # 如果是需要裁剪的图片
            width, height = img.size
            img = img.crop((0, 0, width // 2, height))  # 裁剪图片宽度的一半
        elif prefix in ['center-aside_', 'side-instrument-front-view_']:  # 如果是需要裁剪的图片
            width, height = img.size
...
folders_and_prefixes = {
    'center': 'center_',
    'center_aside_view': 'center-aside_',
    'side': 'side_',
    'side_view_front': 'side-instrument-front-view_'
}
...
# 假设每个文件夹中的图片数量相同
num_images = len(os.listdir('center'))  # 用'center'文件夹作为参考
